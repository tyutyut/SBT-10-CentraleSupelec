...
public class GameController : MonoBehaviour
{
    ...
    public float alpha, beta, gamma, omega, A, B, C, psi; // Parameters for HDC equation
    ...
    private float spawn_x_Value;  // Equals to x(t) ~Xn
    private int score;
    ...
    {                   // ATTENTION la racine choisie n'est peut être pas la bonne... Il faut voir quelle racine considérer si le polynôme en a plusieurs
        if (!paused)
        {
            MathNet.Numerics.Tuple<double, double, double> roots = Root_Newton_Polynomial_HDC(Xn_1, Xn_2, Time.deltaTime, alpha, beta, gamma, omega, A, B, C, psi, mu);
    ...
    float Polynomial_HDC (float Xn, float Xn_1, float Xn_2, float dt, float alpha, float beta, float gamma, float omega, float A, float B, float C, float psi, int mu) // Returns the value of the polynomial from HDC equation, evaluated at Xn (as ugly to see as to type)
    {
        ...
        float coeff_X1 = 1 / (dt * dt) + alpha * (2 * beta * Xn_1 * Xn_1 / (dt * dt) + beta * Xn_1 * Xn_1 / (dt * dt) - gamma) / dt + omega * omega - 2 * mu * beta * rb.position.x * (Xn_1 / dt + mu * rb.velocity.x) - (A + B * rb.position.x * rb.position.x) / dt - C * Mathf.Cos(psi) / dt;
        float coeff_X0 = (Xn_2 - 2 * Xn_1) / (dt * dt) - alpha * Xn_1 * (beta * Xn_1 * Xn_1 / (dt * dt) - gamma) / dt + (A + B * rb.position.x * rb.position.x) * (Xn_1 / dt + mu * rb.velocity.x) - C * (Mathf.Cos(psi) * (-Xn_1 / dt - rb.velocity.x) + Mathf.Sin(psi) * omega * rb.position.x);
        return coeff_X3 * Xn * Xn * Xn + coeff_X2 * Xn * Xn + coeff_X1 * Xn + coeff_X0;
    }
    
    MathNet.Numerics.Tuple<double, double, double> Root_Newton_Polynomial_HDC (float Xn_1, float Xn_2, float dt, float alpha, float beta, float gamma, float omega, float A, float B, float C, float psi, int mu) // Returns the root of the polynomial from HDC Equation (one of these roots is the new value of spawning for the x axis)
    {
        float coeff_X3 = alpha * (1 + beta / (dt * dt)) / dt - beta / dt;
        float coeff_X2 = alpha * (-Xn_1 - 2 * beta * Xn_1 / (dt * dt)) / dt + beta * (Xn_1 / dt + mu * rb.velocity.x) + 2 * mu * beta * rb.position.x / dt;  //rb.position.x is the player x position, and rb.velocity.x its x velocity
        float coeff_X1 = 1 / (dt * dt) + alpha * (2 * beta * Xn_1 * Xn_1 / (dt * dt) + beta * Xn_1 * Xn_1 / (dt * dt) - gamma) / dt + omega * omega - 2 * mu * beta * rb.position.x * (Xn_1 / dt + mu * rb.velocity.x) - (A + B * rb.position.x * rb.position.x) / dt - C * Mathf.Cos(psi) / dt;
